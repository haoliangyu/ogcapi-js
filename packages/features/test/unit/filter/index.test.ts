...
test('stringifyFilter() should return the passed string for string input', () => {
  expect(stringifyFilter({ filter: 'PROPERTY_A = 3' }).filter).toBe(
    'PROPERTY_A = 3'
  );
});
...
test('stringifyFilter() should return the passed uri for filter crs', () => {
  ...
  ).toBe('http://www.opengis.net/def/crs/OGC/1.3/CRS84');
});
...
test('stringifyFilter() should throw an error for invalid filter', () => {
  ...
  expect(() => stringifyFilter({ filter: [] as any })).toThrow(
    'invalid filter'
  );
  expect(() => stringifyFilter({ filter: true as any })).toThrow(
    'invalid filter'
  );
});
